...
if (version_compare(PHP_VERSION, '5.3.7', '<')) {
    exit("Desculpe, Esse sistema de Login e Senha Security PHP Login não é suportado na sua versão PHP atual VPHP > 5.3.7 !");
...
// ... Perguntar se foi realizado o login: (VALIDAÇÃO)
if ($login->isUserLoggedIn() == true) {
    // Se o usuario estiver realizado o login incluir a visualização da pagina em questão
    // Visualizar a pagina para login  Esse tipo de verificação realiza a validação formal do servidor evitando assim o ataque XSS;
	include("admin/sessao.php");
} else {
    // Se o usuario estiver realizado o login incluir a visualização da pagina em questão
    // Visualizar a pagina para login  Esse tipo de verificação realiza a validação formal do servidor evitando assim o ataque XSS;
    include("admin/naologado.php");
}
